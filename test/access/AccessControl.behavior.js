...
function shouldBehaveLikeAccessControl(
  ...
  describe("default admin", function () {
    it("deployer has default admin role", async function () {
      expect(
        await this.accessControl.hasRole(DEFAULT_ADMIN_ROLE, admin)
      ).to.equal(true);
    });

    it("other roles's admin is the default admin role", async function () {
      expect(await this.accessControl.getRoleAdmin(ROLE)).to.equal(
        DEFAULT_ADMIN_ROLE
      );
    });
    ...
    it("default admin role's admin is itself", async function () {
      expect(
        await this.accessControl.getRoleAdmin(DEFAULT_ADMIN_ROLE)
  ...
  describe("granting", function () {
    ...
    it("accounts can be granted a role multiple times", async function () {
      await this.accessControl.grantRole(ROLE, authorized, { from: admin });
      const receipt = await this.accessControl.grantRole(ROLE, authorized, {
        from: admin,
      });
  ...
  describe("revoking", function () {
    it("roles that are not had can be revoked", async function () {
      expect(await this.accessControl.hasRole(ROLE, authorized)).to.equal(
        false
      );

      const receipt = await this.accessControl.revokeRole(ROLE, authorized, {
        from: admin,
      });
  ...
  describe("renouncing", function () {
    ...
    context("with granted role", function () {
      ...
      it("a role can be renounced multiple times", async function () {
        await this.accessControl.renounceRole(ROLE, authorized, {
          from: authorized,
        });
  ...
  describe("setting role admin", function () {
    beforeEach(async function () {
      ...
      await this.accessControl.grantRole(OTHER_ROLE, otherAdmin, {
        from: admin,
      });
    });
    ...
    it("the new admin can revoke roles", async function () {
      await this.accessControl.grantRole(ROLE, authorized, {
        from: otherAdmin,
      });
      const receipt = await this.accessControl.revokeRole(ROLE, authorized, {
        from: otherAdmin,
...
function shouldBehaveLikeAccessControlEnumerable(
  ...
  describe("enumerating", function () {
    it("role bearers can be enumerated", async function () {
      ...
      await this.accessControl.grantRole(ROLE, otherAuthorized, {
        from: admin,
      });
    ...
    it("role enumeration should be in sync after renounceRole call", async function () {
      expect(
        await this.accessControl.getRoleMemberCount(ROLE)
