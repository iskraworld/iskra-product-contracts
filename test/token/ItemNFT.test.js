...
const { shouldNotSupportInterfaces, shouldSupportInterfaces } = require("./ERC165/SupportInterface.behavior");
...
describe("ItemNFT", function () {
  ...
  beforeEach(async function () {
    ...
    this.token = this.contract = await ItemNFT.connect(this.signers[0]).deploy(name, symbol, uri, true);
  });
  ...
  describe("setBaseURI", function () {
    ...
    beforeEach(async function () {
      ...
      expect(await this.contract.mint(other.address, firstTokenId)).emit(this.contract, "Transfer");
    });

    it("changes base uri and emits the eventlog", async function () {
      await expect(this.contract.setBaseURI(newURI)).not.reverted
      expect(await this.contract.tokenURI(1)).to.be.equal(newURI + firstTokenId);
    });

    it("reverts when the sender is not owner", async function () {
      await expect(this.contract.connect(other).setBaseURI(newURI)).revertedWith("Ownable: caller is not the owner");
  ...
  describe("mint", function () {
    ...
    it("reverts with a null destination address", async function () {
      await expect(contract.mint(AddressZero, firstTokenId)).revertedWith("ERC721: mint to the zero address");
    });

    it("reverts if tx sender is not the owner", async function () {
      await expect(contract.connect(other).mint(other.address, firstTokenId)).revertedWith(
  ...
  describe("burn", function () {
    ...
    it("burns properly if tx sender has permission", async function () {
      await expect(contract.connect(owner).setBurnPermissionApproval(other.address, true)).not.reverted;
    ...
    it("reverts if the sender does not have permission", async function () {
      await expect(contract.connect(other).burn(firstTokenId))
          .revertedWith("ItemNFT: the sender does not have permission to burn");
    });

    it("reverts if burning non-owned token", async function () {
      await expect(contract.connect(owner).burn(secondTokenId))
  ...
  describe("setBurnPermissionApproval", function () {
    ...
    beforeEach(async function () {
      ...
      expect(await contract.mint(other.address, firstTokenId)).emit(this.contract, "Transfer");
    });
    ...
    it("reverts when the sender is not owner", async function () {
      await expect(contract.connect(other).setBurnPermissionApproval(other.address, true))
    ...
    it("reverts when the token is not burnable", async function () {
      ...
      await expect(contract.connect(owner).setBurnPermissionApproval(other.address, true))
          .revertedWith("ItemNFT: the token is not burnable");
    });
  });
